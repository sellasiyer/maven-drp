...
/**
 * JUnit test for {@link PCServicesSupportProtectionPage}.
 */
public class PCServicesSupportProtectionPageTest extends BaseNavPageTest {
	...
	assertAccessAllowed(PCServicesSupportProtectionPage.class, DrpConstants.DRP_ADMIN, DrpConstants.DRP_MANAGER,
		DrpConstants.DRP_LEAD, DrpConstants.DRP_USER, DrpConstants.DRP_TEAM);
	assertAccessDenied(PCServicesSupportProtectionPage.class, DrpConstants.DRP_BEAST, DrpConstants.SHP_USER);
    }
	...
	tester.startPage(PCServicesSupportProtectionPage.class, new PageParameters());

	// Check that the right page was rendered (no unexpected redirect or
	// intercept)
	tester.assertRenderedPage(PCServicesSupportProtectionPage.class);
